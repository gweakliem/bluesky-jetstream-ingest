...
from jetstream_ingestion.ingestor import JetstreamIngestor
...
async def test_process_message():
    ...
    with patch('jetstream_ingestion.ingestor.DuckDBWriter', return_value=mock_writer):
        ingestor = JetstreamIngestor('/tmp/test.db', batch_size=2)
...
async def test_flush_messages():
    ...
    with patch('jetstream_ingestion.ingestor.DuckDBWriter', return_value=mock_writer):
        ingestor = JetstreamIngestor('/tmp/test.db')
